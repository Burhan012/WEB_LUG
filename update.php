...
require "connect.php";
$id = $_GET["id"];
$result = mysqli_query($conn, "SELECT * FOM mhs WHERE id = $id");
$mahasiswa = msqli_fetch_assoc($result);

if (isset ($_POST["batal"])){
  header("location: read.php");
}elseif(isset($POST["simpan"])){
  $id = $_POST["id"];
  $nim = htmlspecialchars($_POST['nim']);
  $nama = htmlspecialchars($_POST['nama']);
  $jurusan = htmlspecialchars($_POST['jurursan']);

  $update = mysqli_query($conn, "UPDATE mahasiswa SET `nim`='$nim', `nama`='$nama', `jurusan`='$jurursan' WHERE id = $id");
id($update) {header("location: read.php")
}
    ...
    <input type="hidden" name="id" value="<?= $mhs['id']?>">
      ...
      <div class="form-group">
        ...
        <select class="form-control" name="jurusan">
          ...
          <option value="S1 Akuntansi" <?php echo ($jurusan == 'S1 Akuntasni' ? 'selected' : '')?>>S1 Akuntansi</option>
        </select>
      </div>
